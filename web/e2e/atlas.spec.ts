...
test('mobile atlas reflows without document overflow or control collisions', async ({ page }, testInfo) => {
  ...
  await expectPlotFitsViewport(page, '.program-chart')
  await expectChartTooltipContained(page, '.program-chart')
  const mobileProgramTooltip = page.locator('.program-chart .atlas-chart-tooltip')
  await page.mouse.move(1, 1)
  await expect(mobileProgramTooltip).toBeHidden()
  await clickProgramHeatmapCell(page, 'profile', 1, 0)
  await expect(page.locator('.analysis-context')).toHaveAttribute('data-selected-cluster', '1')
  await expect(mobileProgramTooltip).toBeVisible()
  await page.mouse.move(1, 1)
  await expect(mobileProgramTooltip).toBeHidden()
  ...
  await expectPlotFitsViewport(page, '.program-chart')
  await clickProgramHeatmapCell(page, 'similarity', 1, 10)
  await expect(page.locator('.analysis-context')).toHaveAttribute('data-selected-comparison', /.+/)
  await expect(mobileProgramTooltip).toBeVisible()
  await page.mouse.move(1, 1)
  await expect(mobileProgramTooltip).toBeHidden()
  await expect(page.locator('.programs-view')).toHaveAttribute('data-similarity-x-label-count', '21')
  await expect(page.locator('.programs-view')).toHaveAttribute('data-similarity-x-label-strategy', 'dom-vertical-all')
  ...
  const mobileSimilarityLegendButtons = mobileSimilarityLegend.getByRole('button')
  await expect(mobileSimilarityLegendButtons).toHaveCount(5)
  await expect(mobileSimilarityLegendButtons.first()).toHaveAttribute('aria-pressed', 'true')
  const similarityCanvas = page.locator('.program-chart canvas').last()
  ...
  await expect(mobileProgramXItems).toHaveCount(21)
  await expect(mobileProgramXItems.first()).toHaveText('L · Economía')
  await expect(mobileProgramXItems.last()).toHaveText('D · Historia Aplicada')
  await expect(mobileProgramXItems.first()).toHaveCSS('font-size', '8px')
  const mobileProgramXCenters = await mobileProgramXItems.evaluateAll((items) => items.map((item) => {
